<?php
	...
	sns_header('Prescribir Paciente');
?>
...
<ol class="breadcrumb link-patients">
  ...
  <li class="active">Prescribir Paciente</li>
</ol>
	<?php 
		...
		echo"<div class='alert alert-info' role='alert'>Este reporte ahora esta asignado a ti!</div>";
		}else if($engaged_status!=""&&$signed_status==""){
		$engaged_by=report_info("engaged_by",$id);
		$engaged_by_name= staff_info("full_name",$engaged_by);
		echo"<div class='alert alert-warning' role='alert'>Este reporte esta asignado a $engaged_by_name (ID# $engaged_by) pero aun no ha sido firmado!</div>";
		}else if($signed_status!=""){
		$signed_by_id=report_info("signed_by",$id);	
		$signed_by=staff_info("full_name",$signed_by_id);
		echo"<div class='alert alert-success' role='alert'>Firmado por <a class='staff' href='../staff/profile.php?id=$signed_by_id'>$signed_by</a> (ID# $signed_by_id)!</div>";
		}else{
		echo"";
		}
...
if(isset($_POST['submit'])){
	...
	echo"<div class='alert alert-success' role='alert'>Reporte ha sido exitosamente actualizado!</div>";
		
	}else{
	echo"<div class='alert alert-danger' role='alert'>Lo lamentamos pero este medicamento no esta disponible en el stock~</div>";
...
?>
    ...
    <?php if(report_info("blood_pressure",$id)!=""){?><tr><td>Presion Arterial:</td><td><?php echo report_info("blood_pressure",$id);?></td></tr><?php }?>
    <?php if(report_attachment("exist",$id)==true){?><tr><td>Reportes:</td><td><a href="../media/reports/<?php echo "$id"?>.zip">Descargar adjuntos</a></td></tr><?php }?>
	...
	?>
    <tr><td>- <?php echo $list_med['medicine']?></td><td><?php echo $list_med['doses']?> (<?php echo $list_med['timings']?>) por <?php echo $list_med['days']?> dia(s)</td></tr>
    <?php } ?>
    </tbody></table>

<form method="post" action="" enctype="multipart/form-data">
	<div class="form-group"><label>Medicina:</label><select class="form-control" name='medicine'  id='medicine' size='1' tabindex='1'>
	<?php 
    ...
    <div class="form-group"><label>Dosis:</label><select class="form-control" name='doses' id='doses' size='1'>
          ...
          </select><i>Manana+Tarde+Noche</i></div>
    <div class="form-group"><label>Horas:</label><select class="form-control" name='timings' id='timings' size='1'>
			<option value="After Meal">Despues de la Comida</option>
			<option value="Before Meal">Antes de la Comida</option>
          </select></div>
    <div class="form-group"><label>Dias:</label><select class="form-control" name='days' id='days' size='1'>
    ...
    <div class="form-group"><label>Cargo por Medicina:</label><select class="form-control" name='medicine_charge' id='medicine_charge' size='1'>
			<option value="no">Gratis</option>
			<option value="yes">Cargo</option>
          </select></div>

	<div class="details-clear">&nbsp;</div>

         
	<div class="form-group"><label>Agregar Nota(s):</label><textarea class="form-control" name="reply" id="reply" class="inputOne" cols="9" rows="4"><?php echo report_info("reply",$id);?></textarea></div>
    ...
    <div class="panel-body">
    <table class="table table-striped link-patients link-patients"><tbody>
    <tr><td>Paciente:</td><td><a href="../patients/profile.php?id=<?php echo patient_info("id",$patient_id);?>" class="patient"><?php echo patient_info("name",$patient_id);?></a> | <?php echo patient_info("serial",$patient_id);?>-<?php echo patient_info("id",$patient_id);?></td></tr>
    <tr><td>Registrado:</td><td><?php echo $global_permission->guardian_short_name; echo patient_info("branch",$patient_id);?> - <?php echo branch_name(patient_info("branch",$patient_id));?></td></tr>
    <tr><td>Registrado por:</td><td><a class="staff" href="../staff/profile.php?id=<?php echo patient_info("physician",$patient_id);?>"><?php echo staff_info("full_name",patient_info("physician",$patient_id));?></a></td></tr>
    <tr><td>Ultima Actualizacion:</td><td><?php echo display_time(patient_info("last_update",$patient_id));?></td></tr>
    </tbody></table>
	</div></div>
