<?php
	...
	sns_header('Nuevo Reporte');
?>

<div id="new-report" class="container page">
<div class="panel panel-default">
<div class="panel-heading theme-patients"><span class="inlineicon edit-mini">Nuevo Reporte</span></div>
...
<ol class="breadcrumb link-patients">
  ...
  <li><a href="../patients/register-patients.php">Nuevo Paciente</a></li>
...
if(isset($_POST['submit'])){
	...
	echo"<div class='alert alert-success' role='alert'>Reporte ha sido actualizado exitosamente!</div>";
	
			if(display_permission("prescribe_patient")==true){
				echo"<a class='btn btn-default formbutton theme-patients' href=../patients/prescribe.php?id=$result>Prescribir</a>";
			}else{
				echo"<a class='btn btn-default formbutton theme-patients' href=../patients/register-patient.php?id=$id>Nuevo Paciente</a>";
			}
	
	}else{
	echo"<div class='alert alert-danger' role='alert'>Por favor rellena todos los campos!</div>";
...
?>
	<?php if($success==true){?>
	<div class="alert alert-success" role="alert">Paciente exitosamente agregado.</div>
	<?php }?>
...
<form method="post" action="" enctype="multipart/form-data">
	...
	<div class="form-group"><label>Sintomas:</label><textarea class="form-control" name="symptoms" id="symptoms" class="inputOne" cols="9" rows="4"></textarea></div>
	<div class="form-group"><label>Fiebre:</label><input class="form-control" name="fever" type="text" /><i>&deg;F</i></div>
	<div class="form-group"><label>Presion Arterial:</label><input class="form-control" name="blood_pressure" type="text" /><i>Systolic BP mmHg / Diastolic BP mmHg</i></div>
	<!--
	...
	-->
	<div class="form-group"><label>Asignar a:</label><select class="form-control" name='engaged_by'  id='engaged_by' size='1' tabindex='1'>
	<?php if( display_permission('prescribe_patient',staff_info('id'))==true) { ?>
	<option value="<?php echo staff_info('id');?>">Yo</option>
	<?php }?>
	<option value="">Ticket Abierto</option>
	<?php 
